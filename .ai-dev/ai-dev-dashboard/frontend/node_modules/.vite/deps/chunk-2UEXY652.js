...
} from "./chunk-FFQFUWD2.js";
import {
  at
} from "./chunk-COPWCGZV.js";
import {
  compileStyles,
  solidStateFill,
  styles2String,
  userNodeOverrides
} from "./chunk-Q3XGTYJ2.js";
import {
  createText,
  getIconSVG
...
// node_modules/mermaid/dist/chunks/mermaid.core/chunk-KYZI473N.mjs
...
async function curvedTrapezoid(parent, node) {
  ...
  const w = Math.max(minWidth, (bbox.width + (node.padding ?? 0) * 2) * 1.25, (node == null ? void 0 : node.width) ?? 0);
  const h = Math.max(minHeight, bbox.height + (node.padding ?? 0) * 2, (node == null ? void 0 : node.height) ?? 0);
  const radius = h / 2;
  const { cssStyles } = node;
  const rc = at.svg(shapeSvg);
  const options = userNodeOverrides(node, {});
...
async function halfRoundedRectangle(parent, node) {
  ...
  const { shapeSvg, bbox } = await labelHelper(parent, node, getNodeClasses(node));
  const w = Math.max(minWidth, bbox.width + (node.padding ?? 0) * 2, (node == null ? void 0 : node.width) ?? 0);
  const h = Math.max(minHeight, bbox.height + (node.padding ?? 0) * 2, (node == null ? void 0 : node.height) ?? 0);
  const radius = h / 2;
  const { cssStyles } = node;
  const rc = at.svg(shapeSvg);
...
async function lean_left(parent, node) {
  ...
  node.labelStyle = labelStyles;
  const { shapeSvg, bbox } = await labelHelper(parent, node, getNodeClasses(node));
  const w = Math.max(bbox.width + (node.padding ?? 0), (node == null ? void 0 : node.width) ?? 0);
  const h = Math.max(bbox.height + (node.padding ?? 0), (node == null ? void 0 : node.height) ?? 0);
  const points = [
    { x: 0, y: 0 },
    { x: w + 3 * h / 6, y: 0 },
    { x: w, y: -h },
        ...
        [...images].map(
          (img) => new Promise((res) => {
            function setupImage() {
              ...
              if (noImgText) {
                const bodyFontSize = ((_a = config.fontSize) == null ? void 0 : _a.toString()) ?? window.getComputedStyle(document.body).fontSize;
...
//# sourceMappingURL=chunk-2UEXY652.js.map
